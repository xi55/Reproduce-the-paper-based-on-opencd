...
class NL_Block(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(torch.unique(sim_map))
        # print(torch.unique(context))
...
class NL_FPN_Block(nn.Module):
    ...
    def __init__(self, in_dim, reduction=True):
        ...
        self.conv_e2 = nn.Sequential(
                        ...
                        torch.nn.BatchNorm2d(in_dim*2, momentum=bn_mom),
                        nn.ReLU(inplace=True),
                        )
...
class NL_FPN(nn.Module):
    def __init__(self, channel_list = [256, 128, 64, 32]):

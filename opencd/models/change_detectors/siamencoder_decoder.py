...
class SiamEncoderDecoder(BaseSegmentor):
    ...
    def postprocess_result(self,
        ...
        if(isinstance(seg_logits, list)):
            res = self.fccdn_postprocess_result(seg_logits, data_samples)
            return res
    ...
    def fccdn_postprocess_result(self,
                           seg_logits: Tensor,
                           data_samples: OptSampleList = None) -> SampleList:
        """ Convert results list to `SegDataSample`.
        Args:
        ...
        Returns:
            ...
            - ``seg_logits``(PixelData): Predicted logits of semantic
                segmentation before normalization.
        """
        seg_logits, seg1, seg2 = seg_logits
        batch_size, C, H, W = seg_logits.shape
        ...
        for i in range(batch_size):
            if not only_prediction:
                ...
                if flip:
                    ...
                    if flip_direction == 'horizontal':
                        i_seg_logits = i_seg_logits.flip(dims=(3, ))
                        i_seg1 = i_seg1.flip(dims=(3, ))
                        i_seg2 = i_seg2.flip(dims=(3, ))
                    else:
                        i_seg_logits = i_seg_logits.flip(dims=(2, ))
                        i_seg1 = i_seg1.flip(dims=(2, ))
                        i_seg2 = i_seg2.flip(dims=(2, ))

                # resize as original shape
                res=[]
                for i_seg in [i_seg1, i_seg2, i_seg_logits]:
                    i_seg = resize(
                        i_seg,
                        size=img_meta['ori_shape'],
                        mode='bilinear',
                        ...
                        warning=False).squeeze(0)
                    res.append(i_seg)
                i_seg1, i_seg2, i_seg_logits = res
            else:
                i_seg_logits = seg_logits[i]
                i_seg1 = i_seg1[i]
                i_seg2 = i_seg2[i]
            ...
            if C > 1:
                i_seg_pred = i_seg_logits.argmax(dim=0, keepdim=True)
                i_seg1_pred = i_seg1.argmax(dim=0, keepdim=True)
                i_seg2_pred = i_seg2.argmax(dim=0, keepdim=True)
            else:
                i_seg_logits = i_seg_logits.sigmoid()
                i_seg1 = i_seg1.sigmoid()
                i_seg2 = i_seg2.sigmoid()
            ...
            data_samples[i].set_data({
                ...
                PixelData(**{'data': i_seg_pred}),
                'i_seg1_pred':
                PixelData(**{'data': i_seg1_pred}),
                'i_seg2_pred':
                PixelData(**{'data': i_seg2_pred})
            })

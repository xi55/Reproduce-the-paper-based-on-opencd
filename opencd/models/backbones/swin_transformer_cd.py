...
class SwinTransformerCD(BaseModule):
    ...
    def forward(self, x1, x2):
        ori = [x1, x2]
        x1, hw_shape1 = self.patch_embed(x1)
        x2, hw_shape2 = self.patch_embed(x2)
        ...
        for i, stage in enumerate(self.stages):
            x1, hw_shape1, out1, out_hw_shape1 = stage(x1, hw_shape1)
            x2, hw_shape2, out2, out_hw_shape2 = stage(x2, hw_shape2)
            if i in self.out_indices:
                norm_layer = getattr(self, f'norm{i}')
                out1 = norm_layer(out1)
                out1 = out1.view(-1, *out_hw_shape1,
                               self.num_features[i]).permute(0, 3, 1,
                                                             2).contiguous()
                out2 = out2.view(-1, *out_hw_shape2,
                               self.num_features[i]).permute(0, 3, 1,
                                                             2).contiguous()
                outs1.append(out1)
                outs2.append(out2)
